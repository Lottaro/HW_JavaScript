...
// Преобразуем JSON в строку затем, что в JSON нет одинаковых объектов, так как они ссылаются на разные ячейки памяти и значит все объекты являются уникальными,  
// даже с одинаковой иформацией. И нужно перевести в строку, чтобы отфильтровать уникальные значения
